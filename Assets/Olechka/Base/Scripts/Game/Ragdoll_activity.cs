...
namespace Olechka
{
    [AddComponentMenu("Olechka scripts / Game / Base / Ragdoll activity")]
    ...
    public class Ragdoll_activity : MonoBehaviour
    {
        ...
        public UnityEvent Ragdoll_On_event = new UnityEvent();

        public UnityEvent Ragdoll_Off_event = new UnityEvent();

        [Space(20)]
        [Header("Основное")]

        [Tooltip("Главный родительский объект")]
        [SerializeField]
        ...
        void Up_ragdoll()
        {
            ...
            if (Physics.Raycast(save_position_hip, Vector3.down, out RaycastHit hitInfo))
            {
                save_position_hip = new Vector3(save_position_hip.x, hitInfo.point.y, save_position_hip.z);
            }

            Main_transform.position = save_position_hip;

            Rigidbody_array[0].transform.position = save_position_hip;

            Quaternion save_rotation_hip = Rigidbody_array[0].transform.rotation;

            Vector3 pos_target_look = Main_transform.position + Main_transform.forward * 1f;
            pos_target_look.y = Rigidbody_array[0].transform.position.y;
        ...
        /// <summary>
        /// Полностью поднялся (Активируется по ивенту с конца анимации)
        /// </summary>
        public void Get_Up_animation_final()
        {
            Ragdoll_event.Invoke(true);
            Ragdoll_Off_event.Invoke();
        }
        #endregion
        ...
        /// </summary>
        public void Auto_add_HitBox_component()
        {
            if (Health_script)
            {
                for (int x = 0; x < Collider_array.Length; x++)
                {
                    if (Collider_array[x].TryGetComponent( out HitBox hitbox))
                    {
                        hitbox.Add_Main_health(Health_script);
                    }
                    else
                    {
                        HitBox hitbox_ = Collider_array[x].gameObject.AddComponent<HitBox>();
                        hitbox_.Add_Main_health(Health_script);
                    }
